...
import week11.DijkstraSP;

import java.util.Arrays;
import java.util.HashSet;
import java.util.LinkedList;

public class DijkstraAstar extends DirectedGraphInList{
    HashSet<String> V, S;
    int [] d;
    int r = -1;
    int idest;
    ...
    public void init(String start, String dest, int[] hVal) {
    ...
    public void shortestPath(String start, String dest, int[] h) {
        init(start, dest, h);
        ...
        while(S.size()<maxNumber) {
            ...
            for(String v : adjacent(u)) {
                ...
                if (wuv + h[vertices.indexOf(v)] < min) {
                    min = wuv + h[vertices.indexOf(v)];
                    minPath = v;
                    System.out.println(">>> min path : " + v + " wuv = " + wuv + " h value : " + h[indexOf(v)]);
                }
            }
                HashSet<String> temp = diff(V,S);

                int dv = d[vertices.indexOf(minPath)];

                int du = d[vertices.indexOf(minPath)];

                if(temp.contains(minPath) && (du+getWeight(u, minPath)) < dv) {
                    d[vertices.indexOf(minPath)] = du+getWeight(u, minPath);
                    prev[vertices.indexOf(minPath)] = u;
                }
            }
        }
...
//    }
    ...
    private int getWeight(String u, String v) {
        int ui= indexOf(u);
        LinkedList<DirectedGraphInList.Node> list = adjacentList.get(ui);
    ...
    public static void main(String[] args) {
        ...
        // to Busan
        int[] h = {310, 330, 190, 80, 220, 310, 200, 0, 50, 40};

        DijkstraAstar g = new DijkstraAstar();
